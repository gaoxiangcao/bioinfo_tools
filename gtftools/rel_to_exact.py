...
class GeneRecord(object):
    def __init__(self, gene_record: dict):
        ...
        self.chromosome = gene_record["chrom"]
        ...
        self.exon_strats = self._fix_str_coord_list(gene_record["exonStarts"])
        # 8355120,8355599,8356246,8358916,8359986,
        # print(self.exon_strats)
        self.exon_strats = [i + 1 for i in self.exon_strats]
    ...
    def get_exon_lengths_of_one_transcript(self):
        ...
        if self.exon_frames:
            if self.transcript_start < self.exon_strats[0]:
                exon_5 = self.exon_strats[0] - self.transcript_start

            if self.transcript_end < self.exon_ends[0]:
                exon_3 = self.exon_ends[0] - self.transcript_end

            for idx in range(self.exon_count):
                # 2, 2, 1
                exon_length = self.exon_ends[idx] - self.exon_strats[idx] + 1
            ...
            if self.strand == "-":
                ...
                # print(f"rev_transcript_lengths = {transcript_lengths}")
...
def parse_database(database: str) -> dict:
    ...
    Parse tsv like table
    ucsc_hg38_genes-and-gene-predictions_NCBI-refseq_refGene_knownGene.tsv
...
def query_transcripts(
    ...
    :param int transcript_rel_coord: relative coordinate must be 1 base
    :param dict database: database dict from [parse database]
    :return:
    """
   ...
   # transcript_rel_coord = 951
    if transcript_id in database:
        ...
        raise KeyError(f"{transcript_id} is not in this database")

    gene_annotation = database[transcript_id]
    # print(f"gene_annotation = {gene_annotation}")
    gr = GeneRecord(gene_annotation)
    gene_relative_coord = gr.get_exon_lengths_of_one_transcript()
    # print(f"gene_relative_coord = {gene_relative_coord}")

    # accumulate_sum
    gene_relative_coord_accumulate = accumulate_sum(gene_relative_coord)
    # print(f"gene_relative_coord_accumulate = {gene_relative_coord_accumulate}")

    # short name
    rel = transcript_rel_coord
    # print(f"rel = {rel}")
    accu_rels = gene_relative_coord_accumulate
        ...
        if accumulate_value < rel:
            ...
            if gr.strand == "+":
                ...
                if idx == 0:
                    ...
                    exact_pos = gr.exon_strats[idx - 1] + interval - 1
                    # break
                ...
                if idx == 0:
                    ...
                    interval = rel - accu_rels[idx - 1]
                    exact_pos = gr.exon_ends[-idx] - interval + 1
                    # break
            break
...
def main(aim_transcripts, database, out_path="successful_query_table.csv"):
    f = open(out_path, "wt")
    f.write("id,transcript_position,chromosome,extract_position")
    # parse database
    ...
    # index = 1
    # 实现核心功能的函数
    for nm_id, relative_coord in transcripts.items():
        nm_id = "_".join(nm_id.split("_")[:-1])
        # print(f"nm_id = {nm_id}")
        # print(f"relative_coord = {relative_coord}")

        try:
            query_result = query_transcripts(
                transcript_id=nm_id,
                transcript_rel_coord=relative_coord,
                database=database)
            # print(query_result)
